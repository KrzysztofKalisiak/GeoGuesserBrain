...
def live_plot(y, time_till_finish, labels, epochs, train_acc, test_acc, figsize=(7,5), save=False):
    ...
    ax2.set_ylabel('Total subregion level accuracy (level 3)', color='b')
...
def plot_predicted_points(shp, d13, total_occurences, df1, cords_list):
    ...
    bounds['minx'] -= x_span * 0.1
    bounds['maxx'] += x_span * 0.1
    bounds['miny'] -= y_span * 0.1
    bounds['maxy'] += y_span * 0.1

    fig, ax = plt.subplots(figsize=(10, 10))

    for i, dff in plot_df.groupby(level=0):
        plot_polygon(dff['geometry'].values[0], facecolor=d13[i], label=i,
                     add_points=False, ax=ax)

    plt.legend(prop={'size': 8})

    # ---- FILTER OUT CORRECTLY CLASSIFIED POINTS ----
    xs, ys, colors = [], [], []

    for (lon, lat), (_, true_country) in zip(cords_list, total_occurences):
        p = Point(lon, lat)
        try:
            geom = plot_df.loc[true_country, 'geometry']
        except:
            continue

        if not geom.contains(p):   # only keep misclassified points
            xs.append(lon)
            ys.append(lat)
            colors.append(d13[true_country])

    ax.scatter(xs, ys, color=colors, edgecolors='black')
...
def precision_level_set(total_occurences, shp, i=1):
    ...
    shp1 = gpd.GeoDataFrame(shp.groupby(['_'.join(x.split("_", i)[:i]) for x in shp.index]
                                        ).apply(lambda x: unary_union(x)), columns=['geometry'])
...
class BRAIN:
    ...
    def prepare_system(self, countries=['AL']):
        ...
        if os.path.isdir('precomputes/%s' % self.name):
            with open('precomputes/%s/full_precompute.npy' % self.name, 'rb') as f:
                self.full_precompute = torch.tensor(np.load(f)).to(self.device)
        else:
    ...
    def prepare_dataloaders(self, force=False):

        if not force:
            if type(self.train_dataset) != 'GeoGuesserSystem.data_handling_classes.GeoBrainDatasetFractured':
        ...
        self.test_dataloader = DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=False, pin_memory=True)
    ...
    def real_output_extract(self, data):
        ...
        dx = data[0]
        if dx.shape[0]>1:
            dx_sel = self.full_precompute[self.mapping[dx], :]
        else:
            dx_sel = self.full_precompute[self.mapping[dx], :].reshape(1, -1)

        Hav_gi_xn = dx_sel[:, self.selected_indexes.long()].to(torch.float64)
        ...
        generic_output['side_tasks'] = [data[5][:, [j]].to(self.device) for j in range(data[5].shape[1])]

        generic_output['special_task_GDP'] = data[5][:, [-1]].to(self.device)
    ...
    def accuracy_calc(self):
        ...
        tt = self.task_summary_test['geolocation'].copy()
        ...
        tt = self.task_summary_train['geolocation'].copy()
        tt = tt.applymap(lambda x: x[:12])
        tt = tt.groupby('real')['pred'].value_counts().unstack(-1)
        tt = tt.reindex(tt.index, axis=1).fillna(0)
    ...
    def train(self, epochs=1, name='Unknown'):
        ...
        variable_names_with_level = sum([[str(k)+'_'+y for y in self.y_variable_names[k]] 
                                         for k in self.y_variable_names], [])
        ...
        for epoch in tqdm.tqdm(range(epochs)):  # loop over the dataset multiple times
            ...
            self.train_loss_granular.append({k:np.array([
                [y.detach().cpu() for y in x[k]] for x in running_loss_detailed
                ]).sum(0) for k in running_loss_detailed[0]})
    ...
    def generate_test_main(self, on='test'):
            ...
            for i, data in tqdm.tqdm(enumerate(loader, 0), total=len(loader)):
                ...
                for auxiliary_level in network_output_y:
                    ...
                    for i, _ in enumerate(self.criterions[auxiliary_level]):
                        ...
                        self.total_sidetasks.append((data[1][0], 
                                                     torch.Tensor([auxiliary_level]*data[1][0].size(0)), 
                                                     torch.Tensor([i]*data[1][0].size(0)), 
                                                     network_output_y[auxiliary_level][i][:, 0], 
                                                     real_y[auxiliary_level][i][:, 0]))
        ...
        if on=='test':
            ...
            if len([x for x in self.tasks_location.keys() if x != 'geolocation'])>0:
                ...
                x2['lvl'] = x2[['aux_lvl','lvl']].apply(lambda x: self.y_variable_names[self.tasks_location[
                    [self.NN.tasks[x['aux_lvl']][i][0] for i in range(len(self.NN.tasks[int(x['aux_lvl'])])) 
                                if self.NN.tasks[x['aux_lvl']][i][0]!='geolocation'][0]][0]][int(x['lvl'])], axis=1)
                ...
                self.task_summary_test = self.side_task_summary.join(geolocation_task).sort_index(axis=1)


            else:
                self.task_summary_test = pd.DataFrame(self.total_occurences, 
                                                columns=pd.MultiIndex.from_tuples([('geolocation', 'real'), ('geolocation', 'pred')]))
        else:
            if len([x for x in self.tasks_location.keys() if x != 'geolocation'])>0:
                x1 = np.concatenate([np.vstack([y.detach().cpu().numpy() for y in x]) for x in self.total_sidetasks], 1)
                ...
                self.task_summary_train = self.side_task_summary.join(geolocation_task).sort_index(axis=1)
            else:
                self.task_summary_train = pd.DataFrame(self.total_occurences, 
                                                columns=pd.MultiIndex.from_tuples([('geolocation', 'real'), ('geolocation', 'pred')]))
        #self.prepare_dataloaders()
    ...
    def asses_photo(self, data):
        ...
        Hav_gi_xn = self.full_precompute[self.mapping[data[1][0]], :][self.selected_indexes.long()]
        Hav_gn_xn = data[1][1]
        distance = Hav_gi_xn-Hav_gn_xn
        ...
        s1['probability'] = network_output_y[self.tasks_location['geolocation'][0]][
            self.tasks_location['geolocation'][1]][0][0].detach().cpu().numpy()

        fig, ax = plt.subplots(1, 2, figsize=(60, 30))

        ax1 = s1.plot(ax=ax[0], column='penalize', cmap='bone', 
                      edgecolors='black', legend=True, linewidth=0.2)
        ax1.scatter(data[1][2], data[1][3], edgecolors='black', linewidth=0.2)
        ax1.legend(title='Loss', fontsize=15)

        ax2 = s1.plot(ax=ax[1], column='probability', cmap='OrRd', 
                      edgecolors='black', legend=True, linewidth=0.2)
    ...
    def gradient_track(self, data, auxiliary_lvl=3, lvl=0, cam_type='ScoreCam', original_photo_path=None):
        ...
        if original_photo_path is None:
            try:
                original_photo_path = self.train_dataloader.dataset.img_dir.iloc[
                    np.where(self.train_dataloader.dataset.id_translator==data[1][0])[0][0]]['path']
            except: 
                original_photo_path = self.test_dataloader.dataset.img_dir.iloc[
                    np.where(self.test_dataloader.dataset.id_translator==data[1][0])[0][0]]['path']
        ...
        cam = types_cam[cam_type](model=im.to('cuda'), 
        ...
        for i, angle in enumerate(['120', '240', '360']):
            ...
            original_photo = Image.open(original_photo_path.replace('*', angle).replace('.txt', '.jpg'))
    ...
    def predict(self, data):
        ...
        with torch.no_grad():
            ...
            model_probabilities = network_output_y[self.tasks_location['geolocation'][0]+1][
                self.tasks_location['geolocation'][1]][0].detach().cpu().numpy()

            idx = np.argpartition(model_probabilities, -1, axis=1)[:, -1:]
            mask = np.zeros_like(model_probabilities, dtype=bool)
            rows = np.arange(model_probabilities.shape[0])[:, None]
            mask[rows, idx] = True
            result = np.where(mask, model_probabilities, 0)
